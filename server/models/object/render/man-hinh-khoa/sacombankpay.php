...
function loadBackgroundImageSacombankPay($anhnen) {
...
function applyWatermarkSacombankPay($image) {
...
function canchinhbdsdSacombankPay($image, $fontsize, $y, $textColor, $font, $text) {
...
function createBlurredNotificationBackgroundSacombankPay($image, $notificationX, $notificationY, $notificationWidth, $notificationHeight) {
...
function createRoundedNotificationSacombankPay($notificationBg, $notificationWidth, $notificationHeight, $radius) {
...
function canchinhgiuaSacombankPay($image, $fontsize, $y, $textColor, $font, $text) {
    $fontSize = $fontsize;
...
function canletraiSacombankPay($image, $fontsize, $y, $textColor, $font, $text, $x_tcb) {
    $fontSize = $fontsize;
    imagettftext($image, $fontSize, 0, (int)$x_tcb, (int)$y, $textColor, $font, $text);
}

function canchinhphaiSacombankPay($image, $fontsize, $y, $textColor, $font, $text, $customX = null) {
    $fontSize = $fontsize;
    $textBoundingBox = imagettfbbox($fontSize, 0, $font, $text);
...
function HomeBarsSacombankPay($image) {
...
function LockScreenIconsSacombankPay($image) {
    $scale = 1.92;
...
function NotificationContentSacombankPay($image, $notificationX, $notificationY, $notificationHeight, $fontPath, $soduchinh, $stk, $sotien, $thoigianchuyen, $tenctk, $noidung, $logoPath, $thoigianthongbao, $cachthuc, $magiaodich, $thoigiantrendt, $thoigiantrenmanhinh,$nhamang) {
    ...
    $bdsdRender = canchinhbdsdSacombankPay($image, 30, $notificationY + 130, imagecolorallocate($image, 255, 255, 255), $fontPath.'Inter-Regular.ttf', 
        "Ngày ".DateTime::createFromFormat('d/m/Y H:i', $thoigianchuyen)->format('d/m/Y H:i')." TK: ".($stk).". PS: ".$cachthuc.FormatNumber::TD($sotien, 2)." VND. Số dư khả dụng: ".FormatNumber::TD($soduchinh, 2)." VND.".(($cachthuc == '+') ? '' : (($cachthuc == '-') ? '' : '')).
        " ".$noidung." CKN ".$magiaodich." - ".$tenctk." - Ngan hang giao dich...");
    canletraiSacombankPay($image, 34, $notificationY + 72, imagecolorallocate($image, 255, 255, 255), $fontPath.'common/San Francisco/SanFranciscoDisplay-Semibold.otf', 'Sacombank Pay: Thông báo gi...', 213);
    canchinhgiuaSacombankPay($image, 235, 600, imagecolorallocate($image, 255, 255, 255), $fontPath.'common/San Francisco/SanFranciscoDisplay-Semibold.otf', $thoigiantrendt);
    canchinhgiuaSacombankPay($image, 47, 334, imagecolorallocate($image, 255, 255, 255), $fontPath.'common/San Francisco/SanFranciscoDisplay-Semibold.otf', $thoigiantrenmanhinh);
    canchinhphaiSacombankPay($image, 27, $notificationY + 70, imagecolorallocate($image, 228, 220, 204), $fontPath.'common/San Francisco/SanFranciscoDisplay-Semibold.otf', $thoigianthongbao);
    canletraiBidv($image, 30, $notificationY - 1730, imagecolorallocate($image, 255, 255, 255), $fontPath.'common/San Francisco/SanFranciscoDisplay-Semibold.otf', $nhamang, 113);
    return $bdsdRender['lineCount'];
}

if (isset($_POST['action']) && $_POST['action'] === 'sacombankpay-man-hinh-khoa') {
    $tenctk = $_POST["tenctk"];
    $stk = $_POST["stk"];
    ...
    $logoSacombankPay = 'sacombank.png';
    $iconBank = $_SERVER['DOCUMENT_ROOT'].'/'.__IMG__.'/icon/bank/2/'.$logoSacombankPay;
    $anhnen = $_POST['avatar'] ?? '';
    ...
    $image = loadBackgroundImageSacombankPay($anhnen);
    ...
    $lineCount = NotificationContentSacombankPay($tempImage, $notificationX, $notificationY, $baseHeight, $fontPath, $soduchinh, $stk, $sotien, $thoigianchuyen, $tenctk, $noidung, $iconBank, $thoigianthongbao, $cachthuc, $magiaodich, $thoigiantrendt, $thoigiantrenmanhinh,$nhamang);
    imagedestroy($tempImage);
    $notificationHeight = $baseHeight + ($lineCount - 1) * $lineSpacing;
    $notificationY = imagesy($image) - $notificationHeight - 360;
    $radius = 60;
    $notificationBg = createBlurredNotificationBackgroundSacombankPay($image, $notificationX, $notificationY, $notificationWidth, $notificationHeight);
    $roundedNotification = createRoundedNotificationSacombankPay($notificationBg, $notificationWidth, $notificationHeight, $radius);
    ...
    NotificationContentSacombankPay($image, $notificationX, $notificationY, $notificationHeight, $fontPath, $soduchinh, $stk, $sotien, $thoigianchuyen, $tenctk, $noidung, $iconBank, $thoigianthongbao, $cachthuc, $magiaodich, $thoigiantrendt, $thoigiantrenmanhinh,$nhamang);
    HomeBarsSacombankPay($image);
    LockScreenIconsSacombankPay($image);
    CaiDatPhuTro($image, $vachsong, $iconTinHieu, $iconPins, $_POST["tin-hieu"],$iconDynamicsIsland);
    $image = applyWatermarkSacombankPay($image);
    ob_start();
    imagepng($image);
    $imageData = ob_get_clean();
    $base64 = base64_encode($imageData);
    imagedestroy($image);
    $namebill = ['SacombankPay', 'Màn Hình Khoá'];
