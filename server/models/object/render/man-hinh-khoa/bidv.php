...
function loadBackgroundImageBidv($anhnen) {
...
function applyWatermarkBidv($image) {
...
function canchinhbdsdBidv($image, $fontsize, $y, $textColor, $font, $text) {
...
function createBlurredNotificationBackgroundBidv($image, $notificationX, $notificationY, $notificationWidth, $notificationHeight) {
...
function createRoundedNotificationBidv($notificationBg, $notificationWidth, $notificationHeight, $radius) {
...
function canchinhgiuaBidv($image, $fontsize, $y, $textColor, $font, $text) {
    $fontSize = $fontsize;
...
function canletraiBidv($image, $fontsize, $y, $textColor, $font, $text, $x_tcb) {
    $fontSize = $fontsize;
    imagettftext($image, $fontSize, 0, (int)$x_tcb, (int)$y, $textColor, $font, $text);
}

function canchinhphaiBidv($image, $fontsize, $y, $textColor, $font, $text, $customX = null) {
    $fontSize = $fontsize;
    $textBoundingBox = imagettfbbox($fontSize, 0, $font, $text);
...
function LockScreenIconsBidv($image) {
    $scale = 1.92;
...
function NotificationContentBidv($image, $notificationX, $notificationY, $notificationHeight, $fontPath, $soduchinh, $stk, $sotien, $thoigianchuyen, $tenctk, $noidung, $logoPath, $thoigianthongbao, $cachthuc, $magiaodich, $thoigiantrendt, $thoigiantrenmanhinh,$nhamang) {
    ...
    $bdsdRender = canchinhbdsdBidv($image, 30, $notificationY + 130, imagecolorallocate($image, 255, 255, 255), $fontPath.'Inter-Regular.ttf', 
        ''.DateTime::createFromFormat('H:i - d/m/Y', $thoigianchuyen)->format('H:i d/m/Y')." Tài khoản thanh toán ".($stk)." Số tiền:"." ".$cachthuc.FormatNumber::TD($sotien, 2)."VND. Số dư cuối: ".FormatNumber::TD($soduchinh, 2)."VND ".(($cachthuc == '+') ? '' : (($cachthuc == '-') ? '' : '')).
        "Nội dung giao dịch: ".$noidung);
    canletraiBidv($image, 34, $notificationY + 72, imagecolorallocate($image, 255, 255, 255), $fontPath.'common/San Francisco/SanFranciscoDisplay-Semibold.otf', 'Thông báo BIDV', 213);
    canchinhgiuaBidv($image, 235, 600, imagecolorallocate($image, 255, 255, 255), $fontPath.'common/San Francisco/SanFranciscoDisplay-Semibold.otf', $thoigiantrendt);
    canchinhgiuaBidv($image, 47, 334, imagecolorallocate($image, 255, 255, 255), $fontPath.'common/San Francisco/SanFranciscoDisplay-Semibold.otf', $thoigiantrenmanhinh);
    canchinhphaiBidv($image, 27, $notificationY + 70, imagecolorallocate($image, 228, 220, 204), $fontPath.'common/San Francisco/SanFranciscoDisplay-Semibold.otf', $thoigianthongbao);
...
if (isset($_POST['action']) && $_POST['action'] === 'bidv-man-hinh-khoa') {
    $tenctk = $_POST["tenctk"];
    $stk = $_POST["stk"];
    ...
    $logoBidv = 'Bidv.png';
    $iconBank = $_SERVER['DOCUMENT_ROOT'].'/'.__IMG__.'/icon/bank/2/'.$logoBidv;
    $anhnen = $_POST['avatar'] ?? '';
    ...
    $image = loadBackgroundImageBidv($anhnen);
    ...
    $lineCount = NotificationContentBidv($tempImage, $notificationX, $notificationY, $baseHeight, $fontPath, $soduchinh, $stk, $sotien, $thoigianchuyen, $tenctk, $noidung, $iconBank, $thoigianthongbao, $cachthuc, $magiaodich, $thoigiantrendt, $thoigiantrenmanhinh,$nhamang);
    imagedestroy($tempImage);
    $notificationHeight = $baseHeight + ($lineCount - 1) * $lineSpacing;
    $notificationY = imagesy($image) - $notificationHeight - 360;
    $radius = 60;
    $notificationBg = createBlurredNotificationBackgroundBidv($image, $notificationX, $notificationY, $notificationWidth, $notificationHeight);
    $roundedNotification = createRoundedNotificationBidv($notificationBg, $notificationWidth, $notificationHeight, $radius);
    ...
    NotificationContentBidv($image, $notificationX, $notificationY, $notificationHeight, $fontPath, $soduchinh, $stk, $sotien, $thoigianchuyen, $tenctk, $noidung, $iconBank, $thoigianthongbao, $cachthuc, $magiaodich, $thoigiantrendt, $thoigiantrenmanhinh,$nhamang);
    HomeBarsBidv($image);
    LockScreenIconsBidv($image);
    CaiDatPhuTro($image, $vachsong, $iconTinHieu, $iconPins, $_POST["tin-hieu"],$iconDynamicsIsland);
    $image = applyWatermarkBidv($image);
    ob_start();
    imagepng($image);
    $imageData = ob_get_clean();
    $base64 = base64_encode($imageData);
    imagedestroy($image);
    $namebill = ['BIDV', 'Màn Hình Khoá'];
