...
function loadBackgroundImageTechcombank($anhnen) {
...
function applyWatermarkTechcombank($image) {
...
function canchinhbdsdTechcombank($image, $fontsize, $y, $textColor, $font, $thoigianchuyen, $stk, $cachthuc, $sotien, $soduchinh, $noidung) {
    $imageWidth = imagesx($image);
    $maxWidth = $imageWidth - 295;
    $x = 213;
    $balanceFormatted = 'Số dư: ' . number_format($soduchinh, 0, ',', ',') . ' VND';
    $noidungFormatted = mb_strtoupper($noidung, 'UTF-8');
    $lines = [
        'Tài Khoản: ' . $stk,
        $balanceFormatted,
        $noidungFormatted,
...
function createBlurredNotificationBackgroundTechcombank($image, $notificationX, $notificationY, $notificationWidth, $notificationHeight) {
...
function createRoundedNotificationTechcombank($notificationBg, $notificationWidth, $notificationHeight, $radius) {
...
function canchinhgiuaTechcombank($image, $fontsize, $y, $textColor, $font, $text) {
    $fontSize = $fontsize;
...
function canletraiTechcombank($image, $fontsize, $y, $textColor, $font, $text, $x_tcb) {
    $fontSize = $fontsize;
    imagettftext($image, $fontSize, 0, (int)$x_tcb, (int)$y, $textColor, $font, $text);
}

function canchinhphaiTechcombank($image, $fontsize, $y, $textColor, $font, $text, $customX = null) {
    $fontSize = $fontsize;
    $textBoundingBox = imagettfbbox($fontSize, 0, $font, $text);
...
function HomeBarsTechcombank($image) {
...
function LockScreenIconsTechcombank($image) {
    $scale = 1.92;
...
function NotificationContentTechcombank($image, $notificationX, $notificationY, $notificationHeight, $fontPath, $soduchinh, $stk, $sotien, $noidung, $thoigianchuyen, $logoPath, $thoigianthongbao, $cachthuc, $thoigiantrendt, $thoigiantrenmanhinh, $nhamang) {
    ...
    $bdsdRender = canchinhbdsdTechcombank(
    ...
    canletraiTechcombank($image,34,$notificationY + 72,imagecolorallocate($image, 255, 255, 255),$fontPath . 'common/San Francisco/SanFranciscoDisplay-Semibold.otf',$cachthuc . ' VND ' . number_format($sotien, 0, ',', ','),213);
    canchinhgiuaTechcombank($image, 235, 600, imagecolorallocate($image, 255, 255, 255), $fontPath . 'common/San Francisco/SanFranciscoDisplay-Semibold.otf', $thoigiantrendt);
    canchinhgiuaTechcombank($image, 47, 334, imagecolorallocate($image, 255, 255, 255), $fontPath . 'common/San Francisco/SanFranciscoDisplay-Semibold.otf', $thoigiantrenmanhinh);
    canchinhphaiTechcombank($image, 27, $notificationY + 70, imagecolorallocate($image, 228, 220, 204), $fontPath . 'common/San Francisco/SanFranciscoDisplay-Semibold.otf', $thoigianthongbao);
    canletraiTechcombank($image, 30, $notificationY - 1790, imagecolorallocate($image, 255, 255, 255), $fontPath.'common/San Francisco/SanFranciscoDisplay-Semibold.otf', $nhamang, 113);

    return $bdsdRender['lineCount']; 
}

if (isset($_POST['action']) && $_POST['action'] === 'techcombank-man-hinh-khoa') {
    $stk = $_POST["stk"];
    ...
    $logoTechcombank = 'techcombank.png';
    $iconBank = $_SERVER['DOCUMENT_ROOT'].'/'.__IMG__.'/icon/bank/2/'.$logoTechcombank;
    $anhnen = $_POST['avatar'] ?? '';
    ...
    $image = loadBackgroundImageTechcombank($anhnen);
    ...
    $lineCount = NotificationContentTechcombank($tempImage, $notificationX, $notificationY, $baseHeight, $fontPath, $soduchinh, $stk, $sotien, $noidung, $thoigianchuyen, $iconBank, $thoigianthongbao, $cachthuc, $thoigiantrendt, $thoigiantrenmanhinh, $nhamang);
    imagedestroy($tempImage);
    $notificationHeight = $baseHeight + ($lineCount - 1) * $lineSpacing;
    $notificationY = imagesy($image) - $notificationHeight - 360;
    $radius = 60;
    $notificationBg = createBlurredNotificationBackgroundTechcombank($image, $notificationX, $notificationY, $notificationWidth, $notificationHeight);
    $roundedNotification = createRoundedNotificationTechcombank($notificationBg, $notificationWidth, $notificationHeight, $radius);
    ...
    NotificationContentTechcombank($image, $notificationX, $notificationY, $notificationHeight, $fontPath, $soduchinh, $stk, $sotien, $noidung, $thoigianchuyen, $iconBank, $thoigianthongbao, $cachthuc, $thoigiantrendt, $thoigiantrenmanhinh, $nhamang);
    HomeBarsTechcombank($image);
    LockScreenIconsTechcombank($image);
    CaiDatPhuTro($image, $vachsong, $iconTinHieu, $iconPins, $_POST["tin-hieu"], $iconDynamicsIsland);
    $image = applyWatermarkTechcombank($image);
    ob_start();
    imagepng($image);
    $imageData = ob_get_clean();
    $base64 = base64_encode($imageData);
    imagedestroy($image);
    $namebill = ['Techcombank', 'Màn Hình Khoá'];
