...
function loadBackgroundImageVietinbank($anhnen) {
...
function applyWatermarkVietinbank($image) {
...
function canchinhbdsdVietinbank($image, $fontsize, $y, $textColor, $font, $thoigianchuyen, $stk, $cachthuc, $sotien, $soduchinh) {
    $imageWidth = imagesx($image);
    $maxWidth = $imageWidth - 295;
    $x = 213; 
    $lines = [
        "Thời gian: " . DateTime::createFromFormat('d/m/Y H:i', $thoigianchuyen)->format('d/m/Y H:i'),
        "Tài khoản: " . $stk,
        "Giao dịch: " . $cachthuc . number_format($sotien, 0, ',', '.') . " VND",
        "Số dư hiện tại: " . number_format($soduchinh, 0, ',', '.') . " VND..."
    ];
    ...
    foreach ($lines as $line) {
        ...
        if ($lineWidth > $maxWidth) {
            $cutLine = $line;
            while ($boundingBox[2] - $boundingBox[0] > $maxWidth) {
                $cutLine = mb_substr($cutLine, 0, -1, 'UTF-8');
                $boundingBox = imagettfbbox($fontsize, 0, $font, $cutLine . '...');
            }
            $line = $cutLine . '...';
        }
...
function createBlurredNotificationBackgroundVietinbank($image, $notificationX, $notificationY, $notificationWidth, $notificationHeight) {
...
function createRoundedNotificationVietinbank($notificationBg, $notificationWidth, $notificationHeight, $radius) {
...
function canchinhgiuaVietinbank($image, $fontsize, $y, $textColor, $font, $text) {
    $fontSize = $fontsize;
...
function canletraiVietinbank($image, $fontsize, $y, $textColor, $font, $text, $x_tcb) {
    $fontSize = $fontsize;
    imagettftext($image, $fontSize, 0, (int)$x_tcb, (int)$y, $textColor, $font, $text);
}

function canchinhphaiVietinbank($image, $fontsize, $y, $textColor, $font, $text, $customX = null) {
    $fontSize = $fontsize;
    $textBoundingBox = imagettfbbox($fontSize, 0, $font, $text);
...
function HomeBarsVietinbank($image) {
...
function LockScreenIconsVietinbank($image) {
    $scale = 1.92;
...
function NotificationContentVietinbank($image, $notificationX, $notificationY, $notificationHeight, $fontPath, $soduchinh, $stk, $sotien, $thoigianchuyen, $logoPath, $thoigianthongbao, $cachthuc, $thoigiantrendt, $thoigiantrenmanhinh, $nhamang) {
    ...
    $bdsdRender = canchinhbdsdVietinbank(
    ...
    canletraiVietinbank($image, 34, $notificationY + 72, imagecolorallocate($image, 255, 255, 255), $fontPath . 'common/San Francisco/SanFranciscoDisplay-Semibold.otf', 'Biến động số dư', 213);
    canchinhgiuaVietinbank($image, 235, 600, imagecolorallocate($image, 255, 255, 255), $fontPath . 'common/San Francisco/SanFranciscoDisplay-Semibold.otf', $thoigiantrendt);
    canchinhgiuaVietinbank($image, 47, 334, imagecolorallocate($image, 255, 255, 255), $fontPath . 'common/San Francisco/SanFranciscoDisplay-Semibold.otf', $thoigiantrenmanhinh);
    canchinhphaiVietinbank($image, 27, $notificationY + 70, imagecolorallocate($image, 228, 220, 204), $fontPath . 'common/San Francisco/SanFranciscoDisplay-Semibold.otf', $thoigianthongbao);
    canletraiVietinbank($image, 30, $notificationY - 1730, imagecolorallocate($image, 255, 255, 255), $fontPath . 'common/San Francisco/SanFranciscoDisplay-Semibold.otf', $nhamang, 113); // Thay canletraiBidv bằng canletraiVietinbank

    return $bdsdRender['lineCount']; 
}

if (isset($_POST['action']) && $_POST['action'] === 'vietinbank-man-hinh-khoa') {
    $stk = $_POST["stk"];
    ...
    $logoVietinbank = 'vietinbank.png';
    $iconBank = $_SERVER['DOCUMENT_ROOT'].'/'.__IMG__.'/icon/bank/2/'.$logoVietinbank;
    $anhnen = $_POST['avatar'] ?? '';
    ...
    $spacingCombo = 14;
    //=============================//
    $avatarData = validateAvatar($anhnen); // Truyền tham số $anhnen
    if ($avatarData) {
    ...
    $image = loadBackgroundImageVietinbank($anhnen);
    ...
    $lineCount = NotificationContentVietinbank($tempImage, $notificationX, $notificationY, $baseHeight, $fontPath, $soduchinh, $stk, $sotien, $thoigianchuyen, $iconBank, $thoigianthongbao, $cachthuc, $thoigiantrendt, $thoigiantrenmanhinh, $nhamang);
    imagedestroy($tempImage);
    $notificationHeight = $baseHeight + ($lineCount - 1) * $lineSpacing;
    $notificationY = imagesy($image) - $notificationHeight - 360;
    $radius = 60;
    $notificationBg = createBlurredNotificationBackgroundVietinbank($image, $notificationX, $notificationY, $notificationWidth, $notificationHeight);
    $roundedNotification = createRoundedNotificationVietinbank($notificationBg, $notificationWidth, $notificationHeight, $radius);
    ...
    NotificationContentVietinbank($image, $notificationX, $notificationY, $notificationHeight, $fontPath, $soduchinh, $stk, $sotien, $thoigianchuyen, $iconBank, $thoigianthongbao, $cachthuc, $thoigiantrendt, $thoigiantrenmanhinh, $nhamang);
    HomeBarsVietinbank($image);
    LockScreenIconsVietinbank($image);
    CaiDatPhuTro($image, $vachsong, $iconTinHieu, $iconPins, $_POST["tin-hieu"], $iconDynamicsIsland);
    $image = applyWatermarkVietinbank($image);
    ob_start();
    imagepng($image);
    $imageData = ob_get_clean();
    $base64 = base64_encode($imageData);
    imagedestroy($image);
    $namebill = ['Vietinbank', 'Màn Hình Khoá'];
