...
function loadBackgroundImageMsb($anhnen) {
...
function applyWatermarkMsb($image) {
...
function canchinhbdsdMsb($image, $fontsize, $y, $textColor, $font, $text) {
...
function createBlurredNotificationBackgroundMsb($image, $notificationX, $notificationY, $notificationWidth, $notificationHeight) {
...
function createRoundedNotificationMsb($notificationBg, $notificationWidth, $notificationHeight, $radius) {
...
function canchinhgiuaMsb($image, $fontsize, $y, $textColor, $font, $text) {
    $fontSize = $fontsize;
...
function canletraiMsb($image, $fontsize, $y, $textColor, $font, $text, $x_tcb) {
    $fontSize = $fontsize;
    imagettftext($image, $fontSize, 0, (int)$x_tcb, (int)$y, $textColor, $font, $text);
}

function canchinhphaiMsb($image, $fontsize, $y, $textColor, $font, $text, $customX = null) {
    $fontSize = $fontsize;
    $textBoundingBox = imagettfbbox($fontSize, 0, $font, $text);
...
function LockScreenIconsMsb($image) {
    $scale = 1.92;
...
function NotificationContentMsb($image, $notificationX, $notificationY, $notificationHeight, $fontPath, $soduchinh, $stk, $sotien, $thoigianchuyen, $tenctk, $noidung, $logoPath, $thoigianthongbao, $magiaodich, $thoigiantrendt, $thoigiantrenmanhinh,$nhamang) {
    ...
    $bdsdRender = canchinhbdsdMsb(
    $image,
    30,
    $notificationY + 130,
    imagecolorallocate($image, 255, 255, 255),
    $fontPath . 'Inter-Regular.ttf',
    'TK: ' . DateTime::createFromFormat('H:i - d/m/Y', $thoigianchuyen)->format('d/m H:i') .
    ' TK: ' . $stk .
    ' VND ' . '(+)' .
    ' ' . number_format($sotien, 0, ',', ',') .
    ' (GD:0,Thue/Phi:0)' .
    ' ND: - ' . $magiaodich . ' - ' . $noidung
);
    canletraiMsb($image, 34, $notificationY + 72, imagecolorallocate($image, 255, 255, 255), $fontPath.'common/San Francisco/SanFranciscoDisplay-Semibold.otf', 'Thông báo biến động số dư', 213);
    canchinhgiuaMsb($image, 235, 600, imagecolorallocate($image, 255, 255, 255), $fontPath.'common/San Francisco/SanFranciscoDisplay-Semibold.otf', $thoigiantrendt);
    canchinhgiuaMsb($image, 47, 334, imagecolorallocate($image, 255, 255, 255), $fontPath.'common/San Francisco/SanFranciscoDisplay-Semibold.otf', $thoigiantrenmanhinh);
    canchinhphaiMsb($image, 27, $notificationY + 70, imagecolorallocate($image, 228, 220, 204), $fontPath.'common/San Francisco/SanFranciscoDisplay-Semibold.otf', $thoigianthongbao);
    canletraiMsb($image, 30, $notificationY - 1790, imagecolorallocate($image, 255, 255, 255), $fontPath.'common/San Francisco/SanFranciscoDisplay-Semibold.otf', $nhamang, 113);
    return $bdsdRender['lineCount'];
}

if (isset($_POST['action']) && $_POST['action'] === 'msb-man-hinh-khoa') {
    $tenctk = $_POST["tenctk"];
    $stk = $_POST["stk"];
    ...
    $logoMsb = 'msb.png';
    $iconBank = $_SERVER['DOCUMENT_ROOT'].'/'.__IMG__.'/icon/bank/2/'.$logoMsb;
    $anhnen = $_POST['avatar'] ?? '';
    ...
    $image = loadBackgroundImageMsb($anhnen);
    ...
    $lineCount = NotificationContentMsb($tempImage, $notificationX, $notificationY, $baseHeight, $fontPath, $soduchinh, $stk, $sotien, $thoigianchuyen, $tenctk, $noidung, $iconBank, $thoigianthongbao, $magiaodich, $thoigiantrendt, $thoigiantrenmanhinh,$nhamang);
    imagedestroy($tempImage);
    $notificationHeight = $baseHeight + ($lineCount - 1) * $lineSpacing;
    $notificationY = imagesy($image) - $notificationHeight - 360;
    $radius = 60;
    $notificationBg = createBlurredNotificationBackgroundMsb($image, $notificationX, $notificationY, $notificationWidth, $notificationHeight);
    $roundedNotification = createRoundedNotificationMsb($notificationBg, $notificationWidth, $notificationHeight, $radius);
    ...
    NotificationContentMsb($image, $notificationX, $notificationY, $notificationHeight, $fontPath, $soduchinh, $stk, $sotien, $thoigianchuyen, $tenctk, $noidung, $iconBank, $thoigianthongbao, $magiaodich, $thoigiantrendt, $thoigiantrenmanhinh,$nhamang);
    HomeBarsMsb($image);
    LockScreenIconsMsb($image);
    CaiDatPhuTro($image, $vachsong, $iconTinHieu, $iconPins, $_POST["tin-hieu"],$iconDynamicsIsland);
    $image = applyWatermarkMsb($image);
    ob_start();
    imagepng($image);
    $imageData = ob_get_clean();
    $base64 = base64_encode($imageData);
    imagedestroy($image);
    $namebill = ['MSB', 'Màn Hình Khoá'];
