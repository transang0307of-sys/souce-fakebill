...
function loadBackgroundImageAgribank($anhnen) {
...
function applyWatermarkAgribank($image) {
...
function canchinhbdsdAgribank($image, $fontsize, $y, $textColor, $font, $text) {
...
function createBlurredNotificationBackgroundAgribank($image, $notificationX, $notificationY, $notificationWidth, $notificationHeight) {
...
function createRoundedNotificationAgribank($notificationBg, $notificationWidth, $notificationHeight, $radius) {
...
function canchinhgiuaAgribank($image, $fontsize, $y, $textColor, $font, $text) {
    $fontSize = $fontsize;
...
function canletraiAgribank($image, $fontsize, $y, $textColor, $font, $text, $x_tcb) {
    $fontSize = $fontsize;
    imagettftext($image, $fontSize, 0, (int)$x_tcb, (int)$y, $textColor, $font, $text);
}

function canchinhphaiAgribank($image, $fontsize, $y, $textColor, $font, $text, $customX = null) {
    $fontSize = $fontsize;
    $textBoundingBox = imagettfbbox($fontSize, 0, $font, $text);
...
function HomeBarsAgribank($image) {
...
function LockScreenIconsAgribank($image) {
    $scale = 1.92;
...
function NotificationContentAgribank($image, $notificationX, $notificationY, $notificationHeight, $fontPath, $soduchinh, $stk, $sotien, $thoigianchuyen, $noidung, $logoPath, $thoigianthongbao, $cachthuc, $thoigiantrendt, $thoigiantrenmanhinh,$nhamang) {
    ...
    $bdsdRender = canchinhbdsdAgribank(
    $image,
    30,
    $notificationY + 130,
    imagecolorallocate($image, 255, 255, 255),
    $fontPath . 'Inter-Regular.ttf',
    "Agribank: " . $thoigianchuyen .
    " Tài khoản " . $stk . ": " . 
    $cachthuc . FormatNumber::TD($sotien, 2) . "VND, " .
    "Nội dung giao dịch: " . $noidung . ". " .
    "Số dư cuối " . FormatNumber::TD($soduchinh, 2) . "VND."
    );
    canletraiAgribank($image, 34, $notificationY + 72, imagecolorallocate($image, 255, 255, 255), $fontPath.'common/San Francisco/SanFranciscoDisplay-Semibold.otf', 'Agribank thông báo', 213);
    canchinhgiuaAgribank($image, 235, 600, imagecolorallocate($image, 255, 255, 255), $fontPath.'common/San Francisco/SanFranciscoDisplay-Semibold.otf', $thoigiantrendt);
    canchinhgiuaAgribank($image, 47, 334, imagecolorallocate($image, 255, 255, 255), $fontPath.'common/San Francisco/SanFranciscoDisplay-Semibold.otf', $thoigiantrenmanhinh);
    canchinhphaiAgribank($image, 27, $notificationY + 70, imagecolorallocate($image, 228, 220, 204), $fontPath.'common/San Francisco/SanFranciscoDisplay-Semibold.otf', $thoigianthongbao);
    canletraiAgribank($image, 30, $notificationY - 1730, imagecolorallocate($image, 255, 255, 255), $fontPath.'common/San Francisco/SanFranciscoDisplay-Semibold.otf', $nhamang, 113);
    return $bdsdRender['lineCount'];
}

if (isset($_POST['action']) && $_POST['action'] === 'agribank-man-hinh-khoa') {
    $stk = $_POST["stk"];
    ...
    $logoAgribank = 'agribank.png';
    $iconBank = $_SERVER['DOCUMENT_ROOT'].'/'.__IMG__.'/icon/bank/2/'.$logoAgribank;
    $anhnen = $_POST['avatar'] ?? '';
    ...
    $image = loadBackgroundImageAgribank($anhnen);
    ...
    $lineCount = NotificationContentAgribank($tempImage, $notificationX, $notificationY, $baseHeight, $fontPath, $soduchinh, $stk, $sotien, $thoigianchuyen, $noidung, $iconBank, $thoigianthongbao, $cachthuc, $thoigiantrendt, $thoigiantrenmanhinh,$nhamang);
    imagedestroy($tempImage);
    $notificationHeight = $baseHeight + ($lineCount - 1) * $lineSpacing;
    $notificationY = imagesy($image) - $notificationHeight - 360;
    $radius = 60;
    $notificationBg = createBlurredNotificationBackgroundAgribank($image, $notificationX, $notificationY, $notificationWidth, $notificationHeight);
    $roundedNotification = createRoundedNotificationAgribank($notificationBg, $notificationWidth, $notificationHeight, $radius);
    ...
    NotificationContentAgribank($image, $notificationX, $notificationY, $notificationHeight, $fontPath, $soduchinh, $stk, $sotien, $thoigianchuyen, $noidung, $iconBank, $thoigianthongbao, $cachthuc, $thoigiantrendt, $thoigiantrenmanhinh,$nhamang);
    HomeBarsAgribank($image);
    LockScreenIconsAgribank($image);
    CaiDatPhuTro($image, $vachsong, $iconTinHieu, $iconPins, $_POST["tin-hieu"],$iconDynamicsIsland);
    $image = applyWatermarkAgribank($image);
    ob_start();
    imagepng($image);
    $imageData = ob_get_clean();
    $base64 = base64_encode($imageData);
    imagedestroy($image);
    $namebill = ['Agribank', 'Màn Hình Khoá'];
