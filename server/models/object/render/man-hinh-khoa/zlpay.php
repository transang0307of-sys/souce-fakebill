...
function loadBackgroundImageZlpay($anhnen) {
...
function applyWatermarkZlpay($image) {
...
function canchinhbdsdZlpay($image, $fontsize, $y, $textColor, $font, $text) {
...
function createBlurredNotificationBackgroundZlpay($image, $notificationX, $notificationY, $notificationWidth, $notificationHeight) {
...
function createRoundedNotificationZlpay($notificationBg, $notificationWidth, $notificationHeight, $radius) {
...
function canchinhgiuaZlpay($image, $fontsize, $y, $textColor, $font, $text) {
    $fontSize = $fontsize;
...
function canletraiZlpay($image, $fontsize, $y, $textColor, $font, $text, $x_tcb) {
    $fontSize = $fontsize;
    imagettftext($image, $fontSize, 0, (int)$x_tcb, (int)$y, $textColor, $font, $text);
}

function canchinhphaiZlpay($image, $fontsize, $y, $textColor, $font, $text, $customX = null) {
    $fontSize = $fontsize;
    $textBoundingBox = imagettfbbox($fontSize, 0, $font, $text);
...
function LockScreenIconsZlpay($image) {
    $scale = 1.92;
...
function NotificationContentZlpay($image, $notificationX, $notificationY, $notificationHeight, $fontPath, $sotien, $tenctk, $logoPath, $thoigianthongbao, $thoigiantrendt, $thoigiantrenmanhinh,$nhamang) {
    ...
    $bdsdRender = canchinhbdsdZlpay($image, 30, $notificationY + 130, imagecolorallocate($image, 255, 255, 255), $fontPath.'Inter-Regular.ttf', 
        "Nhận ".FormatNumber::TD($sotien, 1)." đ "."từ ".$tenctk);
    canletraiZlpay($image, 34, $notificationY + 72, imagecolorallocate($image, 255, 255, 255), $fontPath.'common/San Francisco/SanFranciscoDisplay-Semibold.otf', 'Nhận tiền', 213);
    canchinhgiuaZlpay($image, 235, 600, imagecolorallocate($image, 255, 255, 255), $fontPath.'common/San Francisco/SanFranciscoDisplay-Semibold.otf', $thoigiantrendt);
    canchinhgiuaZlpay($image, 47, 334, imagecolorallocate($image, 255, 255, 255), $fontPath.'common/San Francisco/SanFranciscoDisplay-Semibold.otf', $thoigiantrenmanhinh);
    canchinhphaiZlpay($image, 27, $notificationY + 70, imagecolorallocate($image, 228, 220, 204), $fontPath.'common/San Francisco/SanFranciscoDisplay-Semibold.otf', $thoigianthongbao);
    canletraiZlpay($image, 30, $notificationY - 1910, imagecolorallocate($image, 255, 255, 255), $fontPath.'common/San Francisco/SanFranciscoDisplay-Semibold.otf', $nhamang, 113);
    return $bdsdRender['lineCount'];
}

if (isset($_POST['action']) && $_POST['action'] === 'zlpay-man-hinh-khoa') {
    ...
    $logoZlpay = 'zalopay.png';
    $iconBank = $_SERVER['DOCUMENT_ROOT'].'/'.__IMG__.'/icon/bank/2/'.$logoZlpay;
    $anhnen = $_POST['avatar'] ?? '';
    ...
    $image = loadBackgroundImageZlpay($anhnen);
    ...
    $lineCount = NotificationContentZlpay($tempImage, $notificationX, $notificationY, $baseHeight, $fontPath, $sotien, $tenctk, $iconBank, $thoigianthongbao, $thoigiantrendt, $thoigiantrenmanhinh,$nhamang);
    imagedestroy($tempImage);
    $notificationHeight = $baseHeight + ($lineCount - 1) * $lineSpacing;
    $notificationY = imagesy($image) - $notificationHeight - 360;
    $radius = 60;
    $notificationBg = createBlurredNotificationBackgroundZlpay($image, $notificationX, $notificationY, $notificationWidth, $notificationHeight);
    $roundedNotification = createRoundedNotificationZlpay($notificationBg, $notificationWidth, $notificationHeight, $radius);
    ...
    NotificationContentZlpay($image, $notificationX, $notificationY, $notificationHeight, $fontPath, $sotien, $tenctk, $iconBank, $thoigianthongbao, $thoigiantrendt, $thoigiantrenmanhinh,$nhamang);
    HomeBarsZlpay($image);
    LockScreenIconsZlpay($image);
    CaiDatPhuTro($image, $vachsong, $iconTinHieu, $iconPins, $_POST["tin-hieu"],$iconDynamicsIsland);
    $image = applyWatermarkZlpay($image);
    ob_start();
    imagepng($image);
    $imageData = ob_get_clean();
    $base64 = base64_encode($imageData);
    imagedestroy($image);
    $namebill = ['Zalopay1', 'Màn Hình Khoá'];
