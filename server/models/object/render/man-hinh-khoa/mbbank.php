...
function validateAvatar($anhnen) {
...
function loadBackgroundImageMbbank($anhnen) {
...
function applyWatermarkMbbank($image) {
...
function canchinhbdsdMbbank($image, $fontsize, $y, $textColor, $font, $text) {
...
function createBlurredNotificationBackgroundMbbank($image, $notificationX, $notificationY, $notificationWidth, $notificationHeight) {
...
function createRoundedNotificationMbbank($notificationBg, $notificationWidth, $notificationHeight, $radius) {
...
function canchinhgiuaMbbank($image, $fontsize, $y, $textColor, $font, $text) {
    $fontSize = $fontsize;
...
function canletraiMbbank($image, $fontsize, $y, $textColor, $font, $text, $x_tcb) {
    $fontSize = $fontsize;
    imagettftext($image, $fontSize, 0, (int)$x_tcb, (int)$y, $textColor, $font, $text);
}

function canchinhphaiMbbank($image, $fontsize, $y, $textColor, $font, $text, $customX = null) {
    $fontSize = $fontsize;
    $textBoundingBox = imagettfbbox($fontSize, 0, $font, $text);
...
function LockScreenIconsMbbank($image) {
    $scale = 1.92;
...
function NotificationContentMbbank($image, $notificationX, $notificationY, $notificationHeight, $fontPath, $soduchinh, $stk, $sotien, $thoigianchuyen, $tenctk, $noidung, $logoPath, $thoigianthongbao, $cachthuc, $magiaodich, $thoigiantrendt, $thoigiantrenmanhinh,$nhamang) {
    ...
    $bdsdRender = canchinhbdsdMbbank($image, 30, $notificationY + 130, imagecolorallocate($image, 255, 255, 255), $fontPath.'Inter-Regular.ttf', 
        "TK ".FormatStk($stk)."|GD: ".$cachthuc.FormatNumber::TD($sotien, 2)."VND ".DateTime::createFromFormat('H:i - d/m/Y', $thoigianchuyen)->format('d/m/y H:i')." |SD: ".FormatNumber::TD($soduchinh, 2)."VND|".(($cachthuc == '+') ? 'TU' : (($cachthuc == '-') ? 'DEN' : 'TU')).": ".
        $tenctk." - $stk"."|ND: ".$noidung.' - Ma giao dich/Trace '.$magiaodich);
    canletraiMbbank($image, 34, $notificationY + 72, imagecolorallocate($image, 255, 255, 255), $fontPath.'common/San Francisco/SanFranciscoDisplay-Semibold.otf', 'Thông báo biến động số dư', 213);
    canchinhgiuaMbbank($image, 235, 600, imagecolorallocate($image, 255, 255, 255), $fontPath.'common/San Francisco/SanFranciscoDisplay-Semibold.otf', $thoigiantrendt);
    canchinhgiuaMbbank($image, 47, 334, imagecolorallocate($image, 255, 255, 255), $fontPath.'common/San Francisco/SanFranciscoDisplay-Semibold.otf', $thoigiantrenmanhinh);
    canchinhphaiMbbank($image, 27, $notificationY + 70, imagecolorallocate($image, 228, 220, 204), $fontPath.'common/San Francisco/SanFranciscoDisplay-Semibold.otf', $thoigianthongbao);
    canletraiMbbank($image, 30, $notificationY - 1730, imagecolorallocate($image, 255, 255, 255), $fontPath.'common/San Francisco/SanFranciscoDisplay-Semibold.otf', $nhamang, 113);
    return $bdsdRender['lineCount'];
}

if (isset($_POST['action']) && $_POST['action'] === 'mbbank-man-hinh-khoa') {
    $tenctk = $_POST["tenctk"];
    $stk = $_POST["stk"];
    ...
    $cachthuc = in_array($_POST["cachthuc"] ?? '+', ['-', '+']) ? $_POST["cachthuc"] : '+';
    $magiaodich = $_POST["magiaodich"] ?? 'FT'.WsRandomString::Number(16);
    $logoMB = 'mb-'.(isset($_POST["logomb"]) && in_array($_POST["logomb"], [1, 2]) ? ($_POST["logomb"] == 1 ? 'thuong' : 'tet') : '2') . '.png';
    $iconBank = $_SERVER['DOCUMENT_ROOT'].'/'.__IMG__.'/icon/bank/2/'.$logoMB;
    $anhnen = $_POST['avatar'] ?? '';
    ...
    $image = loadBackgroundImageMbbank($anhnen);
    ...
    $lineCount = NotificationContentMbbank($tempImage, $notificationX, $notificationY, $baseHeight, $fontPath, $soduchinh, $stk, $sotien, $thoigianchuyen, $tenctk, $noidung, $iconBank, $thoigianthongbao, $cachthuc, $magiaodich, $thoigiantrendt, $thoigiantrenmanhinh,$nhamang);
    imagedestroy($tempImage);
    $notificationHeight = $baseHeight + ($lineCount - 1) * $lineSpacing;
    $notificationY = imagesy($image) - $notificationHeight - 360;
    $radius = 60;
    $notificationBg = createBlurredNotificationBackgroundMbbank($image, $notificationX, $notificationY, $notificationWidth, $notificationHeight);
    $roundedNotification = createRoundedNotificationMbbank($notificationBg, $notificationWidth, $notificationHeight, $radius);
    ...
    NotificationContentMbbank($image, $notificationX, $notificationY, $notificationHeight, $fontPath, $soduchinh, $stk, $sotien, $thoigianchuyen, $tenctk, $noidung, $iconBank, $thoigianthongbao, $cachthuc, $magiaodich, $thoigiantrendt, $thoigiantrenmanhinh,$nhamang);
    HomeBarsMbbank($image);
    LockScreenIconsMbbank($image);
    CaiDatPhuTro($image, $vachsong, $iconTinHieu, $iconPins, $_POST["tin-hieu"],$iconDynamicsIsland);
    $image = applyWatermarkMbbank($image);
    ob_start();
    imagepng($image);
    $imageData = ob_get_clean();
    $base64 = base64_encode($imageData);
    imagedestroy($image);
    $namebill = ['MB Bank', 'Màn Hình Khoá'];
