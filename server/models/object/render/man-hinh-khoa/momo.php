...
function loadBackgroundImageMomo($anhnen) {
...
function applyWatermarkMomo($image) {
...
function canchinhbdsdMomo($image, $fontsize, $y, $textColor, $font, $text) {
...
function createBlurredNotificationBackgroundMomo($image, $notificationX, $notificationY, $notificationWidth, $notificationHeight) {
...
function createRoundedNotificationMomo($notificationBg, $notificationWidth, $notificationHeight, $radius) {
...
function canchinhgiuaMomo($image, $fontsize, $y, $textColor, $font, $text) {
    $fontSize = $fontsize;
...
function canletraiMomo($image, $fontsize, $y, $textColor, $font, $text, $x_tcb) {
    $fontSize = $fontsize;
    imagettftext($image, $fontSize, 0, (int)$x_tcb, (int)$y, $textColor, $font, $text);
}

function canchinhphaiMomo($image, $fontsize, $y, $textColor, $font, $text, $customX = null) {
    $fontSize = $fontsize;
    $textBoundingBox = imagettfbbox($fontSize, 0, $font, $text);
...
function LockScreenIconsMomo($image) {
    $scale = 1.92;
...
function drawShortenedSenderText($image, $maxWidth, $x, $y, $color, $font, $tenctk, $fontSize) {
    $prefix = 'Nhận tiền chuyển khoản từ ';
    $fullText = $prefix . $tenctk;

    // Kiểm tra chiều rộng full text
    $bbox = imagettfbbox($fontSize, 0, $font, $fullText);
    $textWidth = abs($bbox[2] - $bbox[0]);

    if ($textWidth <= $maxWidth) {
        // Nếu vừa thì vẽ luôn
        imagettftext($image, $fontSize, 0, $x, $y, $color, $font, $fullText);
        return;
    }

    // Rút gọn phần tên người gửi
    $maxName = $tenctk;
    do {
        $maxName = mb_substr($maxName, 0, -1); // cắt từng ký tự
        $shortText = $prefix . $maxName . '...';
        $bbox = imagettfbbox($fontSize, 0, $font, $shortText);
        $textWidth = abs($bbox[2] - $bbox[0]);
    } while ($textWidth > $maxWidth && mb_strlen($maxName) > 0);

    // Vẽ dòng chữ rút gọn
    imagettftext($image, $fontSize, 0, $x, $y, $color, $font, $shortText);
}

function NotificationContentMomo($image, $notificationX, $notificationY, $notificationHeight, $fontPath, $soduchinh, $stk, $sotien, $thoigianchuyen, $tenctk, $noidung, $logoPath, $thoigianthongbao, $magiaodich, $thoigiantrendt, $thoigiantrenmanhinh,$nhamang, $nganhangchuyen) {
    ...
    $bdsdRender = canchinhbdsdMomo($image, 31,$notificationY + 130,imagecolorallocate($image, 255, 255, 255),$fontPath . 'Inter-Regular.ttf', 
    "Số tiền " . FormatNumber::TD($sotien, 1) ." ₫, kèm lời nhắn: " . $noidung . " " . $magiaodich .".CT tu " . $stk . " " . $tenctk . " tai " . $nganhangchuyen);

    $fontSize = 34;
$font = $fontPath . 'common/San Francisco/SanFranciscoDisplay-Semibold.otf';
$color = imagecolorallocate($image, 255, 255, 255);
$maxWidth = 650; // Điều chỉnh theo chiều rộng khu vực bạn muốn vẽ
$x = 213;
$y = $notificationY + 72;

drawShortenedSenderText($image, $maxWidth, $x, $y, $color, $font, $tenctk, $fontSize);

    canchinhgiuaMomo($image, 235, 600, imagecolorallocate($image, 255, 255, 255), $fontPath.'common/San Francisco/SanFranciscoDisplay-Semibold.otf', $thoigiantrendt);
    canchinhgiuaMomo($image, 47, 334, imagecolorallocate($image, 255, 255, 255), $fontPath.'common/San Francisco/SanFranciscoDisplay-Semibold.otf', $thoigiantrenmanhinh);
    canchinhphaiMomo($image, 27, $notificationY + 70, imagecolorallocate($image, 228, 220, 204), $fontPath.'common/San Francisco/SanFranciscoDisplay-Semibold.otf', $thoigianthongbao);
    canletraiMomo($image, 30, $notificationY - 1730, imagecolorallocate($image, 255, 255, 255), $fontPath.'common/San Francisco/SanFranciscoDisplay-Semibold.otf', $nhamang, 113);
    return $bdsdRender['lineCount'];
}

if (isset($_POST['action']) && $_POST['action'] === 'momo-man-hinh-khoa') {
    $tenctk = $_POST["tenctk"];
    $stk = $_POST["stk"];
    ...
    $nganhangchuyen = $_POST["nganhangchuyen"];
    $magiaodich = $_POST["magiaodich"] ?? ''.WsRandomString::Number(6);
    $logoMomo = 'momo.png';
    $iconBank = $_SERVER['DOCUMENT_ROOT'].'/'.__IMG__.'/icon/bank/2/'.$logoMomo;
    $anhnen = $_POST['avatar'] ?? '';
    ...
    $image = loadBackgroundImageMomo($anhnen);
    ...
    $lineCount = NotificationContentMomo($tempImage, $notificationX, $notificationY, $baseHeight, $fontPath, $soduchinh, $stk, $sotien, $thoigianchuyen, $tenctk, $noidung, $iconBank, $thoigianthongbao, $magiaodich, $thoigiantrendt, $thoigiantrenmanhinh,$nhamang, $nganhangchuyen);
    imagedestroy($tempImage);
    $notificationHeight = $baseHeight + ($lineCount - 1) * $lineSpacing;
    $notificationY = imagesy($image) - $notificationHeight - 360;
    $radius = 60;
    $notificationBg = createBlurredNotificationBackgroundMomo($image, $notificationX, $notificationY, $notificationWidth, $notificationHeight);
    $roundedNotification = createRoundedNotificationMomo($notificationBg, $notificationWidth, $notificationHeight, $radius);
    ...
    NotificationContentMomo($image, $notificationX, $notificationY, $notificationHeight, $fontPath, $soduchinh, $stk, $sotien, $thoigianchuyen, $tenctk, $noidung, $iconBank, $thoigianthongbao, $magiaodich, $thoigiantrendt, $thoigiantrenmanhinh,$nhamang, $nganhangchuyen);
    HomeBarsMomo($image);
    LockScreenIconsMomo($image);
    CaiDatPhuTro($image, $vachsong, $iconTinHieu, $iconPins, $_POST["tin-hieu"],$iconDynamicsIsland);
    $image = applyWatermarkMomo($image);
    ob_start();
    imagepng($image);
    $imageData = ob_get_clean();
    $base64 = base64_encode($imageData);
    imagedestroy($image);
    $namebill = ['Momo', 'Màn Hình Khoá'];
