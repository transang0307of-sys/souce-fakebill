...
function loadBackgroundImageZalopay($anhnen) {
...
function applyWatermarkZalopay($image) {
...
function canchinhbdsdZalopay($image, $fontsize, $y, $textColor, $font, $text) {
...
function createBlurredNotificationBackgroundZalopay($image, $notificationX, $notificationY, $notificationWidth, $notificationHeight) {
...
function createRoundedNotificationZalopay($notificationBg, $notificationWidth, $notificationHeight, $radius) {
...
function canchinhgiuaZalopay($image, $fontsize, $y, $textColor, $font, $text) {
    $fontSize = $fontsize;
...
function canletraiZalopay($image, $fontsize, $y, $textColor, $font, $text, $x_tcb) {
    $fontSize = $fontsize;
    imagettftext($image, $fontSize, 0, (int)$x_tcb, (int)$y, $textColor, $font, $text);
}

function canchinhphaiZalopay($image, $fontsize, $y, $textColor, $font, $text, $customX = null) {
    $fontSize = $fontsize;
    $textBoundingBox = imagettfbbox($fontSize, 0, $font, $text);
...
function HomeBarsZalopay($image) {
...
function LockScreenIconsZalopay($image) {
    $scale = 1.92;
...
function NotificationContentZalopay($image, $notificationX, $notificationY, $notificationHeight, $fontPath, $sotien, $tenctk, $logoPath, $thoigianthongbao, $thoigiantrendt, $thoigiantrenmanhinh,$nhamang) {
    ...
    $bdsdRender = canchinhbdsdZalopay($image, 30, $notificationY + 130, imagecolorallocate($image, 255, 255, 255), $fontPath.'Inter-Regular.ttf', 
        $tenctk ." đã chuyển tiền cho bạn ".FormatNumber::TD($sotien, 1)."đ");
    canletraiZalopay($image, 34, $notificationY + 72, imagecolorallocate($image, 255, 255, 255), $fontPath.'common/San Francisco/SanFranciscoDisplay-Semibold.otf', 'Nhận tiền thành công', 213);
    canchinhgiuaZalopay($image, 235, 600, imagecolorallocate($image, 255, 255, 255), $fontPath.'common/San Francisco/SanFranciscoDisplay-Semibold.otf', $thoigiantrendt);
    canchinhgiuaZalopay($image, 47, 334, imagecolorallocate($image, 255, 255, 255), $fontPath.'common/San Francisco/SanFranciscoDisplay-Semibold.otf', $thoigiantrenmanhinh);
    canchinhphaiZalopay($image, 27, $notificationY + 70, imagecolorallocate($image, 228, 220, 204), $fontPath.'common/San Francisco/SanFranciscoDisplay-Semibold.otf', $thoigianthongbao);
    canletraiZalopay($image, 30, $notificationY - 1850, imagecolorallocate($image, 255, 255, 255), $fontPath.'common/San Francisco/SanFranciscoDisplay-Semibold.otf', $nhamang, 113);
    return $bdsdRender['lineCount'];
}

if (isset($_POST['action']) && $_POST['action'] === 'zalopay-man-hinh-khoa') {
    ...
    $logoZalopay = 'zalopay.png';
    $iconBank = $_SERVER['DOCUMENT_ROOT'].'/'.__IMG__.'/icon/bank/2/'.$logoZalopay;
    $anhnen = $_POST['avatar'] ?? '';
    ...
    $image = loadBackgroundImageZalopay($anhnen);
    ...
    $lineCount = NotificationContentZalopay($tempImage, $notificationX, $notificationY, $baseHeight, $fontPath, $sotien, $tenctk, $iconBank, $thoigianthongbao, $thoigiantrendt, $thoigiantrenmanhinh,$nhamang);
    imagedestroy($tempImage);
    $notificationHeight = $baseHeight + ($lineCount - 1) * $lineSpacing;
    $notificationY = imagesy($image) - $notificationHeight - 360;
    $radius = 60;
    $notificationBg = createBlurredNotificationBackgroundZalopay($image, $notificationX, $notificationY, $notificationWidth, $notificationHeight);
    $roundedNotification = createRoundedNotificationZalopay($notificationBg, $notificationWidth, $notificationHeight, $radius);
    ...
    NotificationContentZalopay($image, $notificationX, $notificationY, $notificationHeight, $fontPath, $sotien, $tenctk, $iconBank, $thoigianthongbao, $thoigiantrendt, $thoigiantrenmanhinh,$nhamang);
    HomeBarsZalopay($image);
    LockScreenIconsZalopay($image);
    CaiDatPhuTro($image, $vachsong, $iconTinHieu, $iconPins, $_POST["tin-hieu"],$iconDynamicsIsland);
    $image = applyWatermarkZalopay($image);
    ob_start();
    imagepng($image);
    $imageData = ob_get_clean();
    $base64 = base64_encode($imageData);
    imagedestroy($image);
    $namebill = ['Zalopay', 'Màn Hình Khoá'];
