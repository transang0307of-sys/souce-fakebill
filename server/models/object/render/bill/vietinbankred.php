...
if (isset($_POST['action']) && $_POST['action'] === 'vietinbankred') {
    ...
    $phoiBank = $_SERVER['DOCUMENT_ROOT'].'/'.__IMG__.'/phoi/bill/vietinbankred.png';
...
function canbentrai($image, $text, $fontPath, $fontSize, $color, $x, $y) {
    // $image     : ảnh tạo bằng imagecreatetruecolor hoặc imagecreatefrom*
    // $text      : nội dung văn bản cần in
    // $fontPath  : đường dẫn đến font (VD: '/path/to/vietinbank/SVN-Gilroy Medium.otf')
    // $fontSize  : cỡ chữ
    // $color     : màu chữ (tạo bằng imagecolorallocate)
    // $x         : tọa độ X (bên trái)
    // $y         : tọa độ Y (baseline chữ)

    $angle = 0; // Không nghiêng
    ...
    canchinhphai($image, $thoigianchuyen, $fontPath.'vietinbank/SVN-Gilroy SemiBold.otf', 27, imagecolorallocate($image, 158,171,176), 490);
    canchinhphai($image, $magiaodich, $fontPath.'vietinbank/SVN-Gilroy SemiBold.otf', 27, imagecolorallocate($image, 158,171,176), 530);
    canchinhphai($image, '********' . substr($stk_nc, -4), $fontPath.'vietinbank/SVN-Gilroy Medium.otf', 31, imagecolorallocate($image, 13, 42, 70), 965);
    canchinhphai($image, $tennguoichuyen, $fontPath.'vietinbank/SVN-Gilroy Medium.otf', 31, imagecolorallocate($image, 13, 42, 70), 1010);
    canchinhphai($image, $stk, $fontPath.'vietinbank/SVN-Gilroy Bold.otf', 31, imagecolorallocate($image, 13, 42, 70), 1150);
    canchinhphai($image, $tennguoinhan, $fontPath.'vietinbank/SVN-Gilroy Bold.otf', 31, imagecolorallocate($image, 13, 42, 70), 1200);
    canchinhnganhang($image, $nganhangnhan, $fontPath.'vietinbank/SVN-Gilroy Medium.otf', 30, imagecolorallocate($image, 13, 42, 70), 1336);
    canchinhphai($image, FormatNumber::TD($sotienchuyen,2).' VND', $fontPath.'vietinbank/SVN-Gilroy XBold.otf', 31, imagecolorallocate($image, 4, 88, 146), 1531);
    canchinhtien($image, convert_number_to_words($sotienchuyen,2), $fontPath.'vietinbank/SVN-Gilroy XBold.otf', 31, imagecolorallocate($image, 4, 88, 146), 1581);
    canchinhphai($image, $phigiaodich, $fontPath.'vietinbank/SVN-Gilroy Medium.otf', 31, imagecolorallocate($image, 13, 42, 70), 1702);
    canchinhnoidung($image, $noidungchuyen, $fontPath.'vietinbank/SVN-Gilroy Medium.otf', 31, imagecolorallocate($image, 13, 42, 70), 1836);
    CaiDatPhuTro($image, $vachsong, $iconTinHieu, $iconPins, $_POST["tin-hieu"],$iconDynamicsIsland);
    
    $font = $fontPath . 'vietinbank/SVN-Gilroy Medium.otf';
    $color = imagecolorallocate($image, 100,127,139);
    // Gọi hàm đã sửa
    canbentrai($image, 'Từ tài khoản', $font, 30, $color, 80, 965);
    canbentrai($image, 'Đến tài khoản', $font, 30, $color, 80, 1150);
    canbentrai($image, 'Ngân hàng', $font, 30, $color, 80, 1336);
    canbentrai($image, 'Số tiền', $font, 30, $color, 80, 1531);
    canbentrai($image, 'Phí', $font, 30, $color, 80, 1702);
    canbentrai($image, 'Nội dung', $font, 30, $color, 80, 1836);
    ob_start();
    imagepng($image);
    ...
    imagedestroy($image);
    imagedestroy($img);
    $namebill = ['Vietinbankred','Chuyển Khoản'];
