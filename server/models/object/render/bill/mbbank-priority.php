...
if (isset($_POST['action']) && $_POST['action'] === 'mbbank-priority') {
    ...
    $phoiBank = $_SERVER['DOCUMENT_ROOT'].'/'.__IMG__.'/phoi/bill/'.($kieuchuyen === 'macdinh' ? 'mb-priority.png' : 'mb-priority-bdsd.png');
    ...
    if (isset($_POST["xem-truoc"]) && $_POST["xem-truoc"] === 'yes') {
        $image = Watermark::Render($image,1.2,33);
    }    
    if (!file_exists($iconBank)) {
        ...
        function cangiua_nganhang($image, $y, $textColor, $font, $iconBank, $name_bank, $fontSize, $iconOffsetY = 5)
        {
            ...
            list($iconWidth, $iconHeight) = ($name_bank === 'ACB') ? [89, 89] : [64, 64];
        ...
        {
            $imageWidth = imagesx($image);
            $maxWidth = $imageWidth - 290;
            $maxWidthSecondLine = $imageWidth - 221;
            $words = explode(' ', $text);
            $currentLine = '';
            $lines = [];
            foreach ($words as $word) {
        ...
        canletrai($image, 37, 84, imagecolorallocate($image, 255,255,255), $fontPath . 'common/San Francisco/SanFranciscoDisplay-Semibold.otf', $thoigiantrendt,130);
        if ($kieuchuyen!=='macdinh') {
        canchinhbdsd($image, 32, 262, imagecolorallocate($image, 255,255,255), $fontPath.'Inter-Regular.ttf', "TK ".FormatStk($stkchinh)."|GD: -".FormatNumber::TD($sotienchuyen, 2)."VND ".DateTime::createFromFormat('H:i - d/m/Y', $thoigianchuyen)->format('d/m/y H:i')." |SD: ".FormatNumber::TD($soduchinh, 2)."VND|DEN: ".$tennguoinhan." - $stk"." |".$noidungchuyen);
        }
        canchinhgiua($image, 60, 740, imagecolorallocate($image, 13,89,165), $fontPath.'common/San Francisco/SanFranciscoDisplay-Semibold.otf', FormatNumber::TD($sotienchuyen, 2).' VND');
        canchinhgiua($image, 28, 820, imagecolorallocate($image, 107,107,107), $fontPath.'common/AvertaStd/AvertaStd-Regular.otf', $thoigianchuyen);
        canchinhgiua($image, 37, 1099, imagecolorallocate($image, 27,48,59), $fontPath.'common/AvertaStd/AvertaStd-Semibold.otf', $tennguoinhan);
        cangiua_nganhang($image, 1187, imagecolorallocate($image, 27,48,59), $fontPath.'common/AvertaStd/AvertaStd-Regular.otf', $iconBank, $name_bank, 32, $name_bank === 'ACB' ? 28 :18);
        canchinhgiua($image, 32, 1274, imagecolorallocate($image, 27,48,59), $fontPath.'common/AvertaStd/AvertaStd-Regular.otf', $stk);
        canchinhgiua($image, 32, 1357, imagecolorallocate($image, 27,48,59), $fontPath.'common/AvertaStd/AvertaStd-Regular.otf', $noidungchuyen);
        ...
        imagedestroy($image);
        $namebill = ['MB Bank Priority','Chuyển Khoản'];
