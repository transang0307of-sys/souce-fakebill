...
if (isset($_POST['action']) && $_POST['action'] === 'vietcombank-new') {
    ...
    // $bdsd= $_POST["bdsd"];
    $phoiBank = $_SERVER['DOCUMENT_ROOT'].'/'.__IMG__.'/phoi/bill/vcb-new.png';
    ...
    // $stk_nc= $_POST["stk-nc"];
    // $kc_master= true;
    // if (empty($stk_nc)||empty($soduchinh)) {
    //     exit(JSON_FORMATTER(["status" => -1, "msg" => "Vui lòng không bỏ trống mục số dư chính hoặc số tài khoản của bạn."]));
    // }
    ...
    if (!file_exists($iconBankPath)) {
        ...
        function canchinhphai_banks($image, $y, $textColor, $font, $iconBankPath, $nganhang_stk, $fullname, $fontSize, $fullnameFont, $fullnameColor, $iconOffsetY = 5, $iconWidth = 40, $iconHeight = 40, $vtBankWidth = 80, $vtBankHeight = 80)
        {
            ...
            $fullnameX = $imageWidth - $fullnameWidth - 98;
            $fullnameY = $y + 60;
        ...
        $vnd = 'VND';
        $textBox = imagettfbbox(54, 0, $fontPath.'vietcombank/Manrope-Bold.ttf', $stc);
        $textWidth = $textBox[2] - $textBox[0];
        $vndBox = imagettfbbox(29, 0, $fontPath.'vietcombank/SVN-Arial 3.ttf', $vnd);
        ...
        imagettftext($image, 54, 0, $xAmount, $yAmount, imagecolorallocate($image, 8, 75, 49), $fontPath.'vietcombank/Manrope-Bold.ttf', $stc);
        imagettftext($image, 29, 0, $xVND, $yVND, imagecolorallocate($image, 106, 148, 131), $fontPath.'vietcombank/SVN-Arial 3.ttf', $vnd);
        canchinhphai($image, 33, 1075, imagecolorallocate($image, 39, 39, 39), $fontPath.'vietcombank/UTM HelveBold.ttf', $tennguoinhan);
        canchinhphai($image, 34, 925, imagecolorallocate($image, 39,39,39), $fontPath.'vietcombank/UTM HelveBold.ttf', $stk);
        canchinhgiua($image, 29, 774, imagecolorallocate($image, 92,92,91), $fontPath.'common/Helvetica/Helvetica.ttf', $thoigianchuyen);
        ...
        canchinhphai_banks($image, 1222, imagecolorallocate($image, 35, 35, 35), $fontPath.'vietcombank/UTM HelveBold.ttf', $iconBankPath, $nganhang_stk, $fullnamebank, 33, $fontPath.'vietcombank/SVN-Arial 3.ttf', imagecolorallocate($image, 48, 48, 48), 22, 39, 39, 72, 72);
        canchinhphai_noidung($image, 33, 1390, imagecolorallocate($image, 39, 39, 39), $fontPath.'vietcombank/UTM HelveBold.ttf', $noidungchuyen, 1.9);
        ...
        // canchinhphai($image, 33, 1790, imagecolorallocate($image, 39,39,39), $fontPath.'vietcombank/UTM HelveBold.ttf', 
        // $kieuchuyen === 'trongbank' ? 
        // "Chuyển tiền trong\n".str_repeat(" ",8)."Vietcombank" : 
        // "Chuyển tiền nhanh\n".str_repeat(" ",24)."24/7"
        //  );
        canchinhphai($image, 34, 2050, imagecolorallocate($image, 39,39,39), $fontPath.'vietcombank/UTM HelveBold.ttf', $magiaodich);
        canchinhphai($image, 33, 1562, imagecolorallocate($image, 39,39,39), $fontPath.'vietcombank/UTM HelveBold.ttf', $mathamchieu, 160);
        ...
        imagedestroy($image);
        $namebill = ['Vietcombank','Chuyển Khoản'];
