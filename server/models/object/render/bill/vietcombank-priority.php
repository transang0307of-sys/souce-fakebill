...
if (isset($_POST['action']) && $_POST['action'] === 'vietcombank-priority') {
    ...
    // $bdsd= $_POST["bdsd"];
    $phoiBank = $_SERVER['DOCUMENT_ROOT'].'/'.__IMG__.'/phoi/bill/vcb1.png';
    ...
    if (isset($_POST["xem-truoc"]) && $_POST["xem-truoc"] === 'yes') {
        ...
        function canlephai_banks($image, $y, $textColor, $font, $iconBankPath, $nganhang_stk, $fullname, $fontSize, $fullnameFont, $fullnameColor, $iconOffsetY = 5, $iconWidth = 40, $iconHeight = 40, $vtBankWidth = 80, $vtBankHeight = 80)
        {
            ...
            // imagesavealpha($iconVtBankResized, false);
            ...
            $fullnameX = $imageWidth - $fullnameWidth - 68;
            $fullnameY = $y + 50;
        ...
        imagettftext($image, 45, 0, $xAmount, $yAmount, imagecolorallocate($image, 233, 202, 156), $fontPath.'vietcombank/Manrope-Bold.ttf', $stc);
        imagettftext($image, 22, 0, $xVND, $yVND, imagecolorallocate($image, 235, 202, 156), $fontPath.'vietcombank/SVN-Arial 3.ttf', $vnd);
        canlephai($image, 25, 748, imagecolorallocate($image, 255, 255, 255), $fontPath.'vietcombank/UTM HelveBold.ttf', $tennguoinhan);
        // canchinhphai($image, 25, 656, imagecolorallocate($image, 255, 255, 255), $fontPath.'vietcombank/UTM HelveBold.ttf', $stk);
        canlephai($image, 25, 656, imagecolorallocate($image, 255, 255, 255), $fontPath . 'vietcombank/UTM HelveBold.ttf', $stk);
        canchinhgiua($image, 22, 525, imagecolorallocate($image, 128, 128, 128), $fontPath.'common/Helvetica/Helvetica.ttf', $thoigianchuyen);
        imagettftext($image, 30, 0, 105, 60, imagecolorallocate($image, 0, 1, 2), $fontPath . 'common/San Francisco/SanFranciscoDisplay-Semibold.otf', $thoigiantrendt);
        // canletrai($image, 26, 83, imagecolorallocate($image, 0, 0, 0), $fontPath.'common/San Francisco/SanFranciscoDisplay-Semibold.otf', $thoigiantrendt, 110);
        canlephai_banks($image, 833, imagecolorallocate($image, 255, 255, 255), $fontPath.'vietcombank/UTM HelveBold.ttf', $iconBankPath, $nganhang_stk, $fullnamebank, 25, $fontPath.'vietcombank/SVN-Arial 3.ttf', imagecolorallocate($image, 255, 255, 255), 17, 29, 29, 52, 52);
        ...
        canlephai($image, 25, 1332, imagecolorallocate($image, 255, 255, 255), $fontPath. 'vietcombank/UTM HelveBold.ttf', $magiaodich);
        ...
        imagedestroy($image);
        $namebill = ['Vietcombank-Priority','Chuyển Khoản'];
